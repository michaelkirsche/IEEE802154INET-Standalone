    ...
    else if (stage == 2)
    {
        ...
        if (true == startNow)
        {
            ...
            if (true == isCoordinator)
            {
                ...
                // use the current (user-defined or auto-generated) MAC 64-bit extended address
                mpib.setMacCoordExtendedAddress(myMacAddr);
                // use the current (user-defined or auto.generated) MAC 16-bit short address
...
void IEEE802154Mac::setDefMpib()
{
    ...
    if (isCoordinator)
    {
        mpib.setMacShortAddress(myMacAddr.getShortAddr());  // use the current (user-defined or auto-generated) MAC 16-bit short address
...
void IEEE802154Mac::handleCommand(mpdu* frame)
{
    ...
    switch (cmdFrame->getCmdType())
    {
        ...
        case Ieee802154_POLL_REQUEST: {
            ...
            {
                genACK(frame->getSqnr(), true); // generate an ACK for the incoming MLME-POLL.request with FramePending = true
                // XXX needed? MLME-POLL.request FCF should include the ACK flag, which gets evaluated (and taken care of) during reception
            }
            else
            {
                genACK(frame->getSqnr(), false); // generate an ACK for the incoming MLME-POLL.request with FramePending = false
                // XXX needed? MLME-POLL.request FCF should include the ACK flag, which gets evaluated (and taken care of) during reception
            }
            rxCmd = NULL;   // delete command message buffer after processing
            break;
...
bool IEEE802154Mac::filter(mpdu* pdu)
{
    ...
    // Filtering Level 3.4: Frame Type == Data || Command --> perform FCF checks
    if ((frameType == Data) || (frameType == Command))
    {
        ...
        if (destAddressMode != noAddr)
        {
            ...
            if ((pdu->getDestPANid() != BROADCAST_SHORT_ADDRESS) && (pdu->getDestPANid() != mpib.getMacPANId()))
            {
                ...
                return true;   // all other cases => filter the frame
            }
        }
...
bool IEEE802154Mac::csmacaCanProceed(simtime_t wtime, bool afterCCA)
{
    ...
    else  // as a device
    {
        if (rxBO != 15)  // beacon enabled as a device
        {
            ...
            /* If max beacon loss is reached, use unslotted version
             rxBI = rxSfSpec.BI / phy_symbolrate;
        ...
        switch (taskP.taskStep(task))
        {
            ...
            case 3: {
                ...
                else  // time out when waiting for ACK
                {
                    ...
                    {
                        macEV << "The max #number of retries (MaxFrameRetries=" << mpib.getMacMaxFrameRetries() << ") reached \n";
        ...
        switch (taskP.taskStep(task))
        {
            ...
            case 3: {
                ...
                else  // time out when waiting for ACK, normally impossible in GTS
                {
                    ...
                    {
                        macEV << "[GTS]: the #max number of retries (MacMaxFrameRetries=" << mpib.getMacMaxFrameRetries() << ") reached, task failed \n";
                        taskP.taskStatus(task) = false;  //task fails
                        macEV << "[MAC-TASK-FAIL]: reset TP_MCPS_DATA_REQUEST \n";
                        waitGTSAck = false;
                        // to avoid several consecutive genSetTRXState are called at the same time, which may lead to error operation,
...
void IEEE802154Mac::finish()
{
    ...
    if (currentTime > 0)
    {
        recordScalar("Number of upper DATA packets received in MAC", numUpperDataPkt);
        recordScalar("Number of upper DATA packets dropped in MAC", numUpperDataPktDropped);
        recordScalar("Number of upper MANAGEMENT packets received in MAC", numUpperMgmtPkt);
        recordScalar("Number of upper MANAGEMENT packets dropped in MAC", numUpperMgmtPktDropped);
        recordScalar("Number of BEACON packets sent", numTxBcnPkt);
        recordScalar("Number of DATA packets sent successfully", numTxDataSucc);
        recordScalar("Number of DATA packets failed", numTxDataFail);
        recordScalar("Number of DATA packets sent successfully in GTS", numTxGTSSucc);
        recordScalar("Number of DATA packets failed in GTS", numTxGTSFail);
        recordScalar("Number of ACK packets sent", numTxAckPkt);
        recordScalar("Number of BEACON packets received", numRxBcnPkt);
        recordScalar("Number of BEACON packets lost", numLostBcn);
        recordScalar("Number of DATA packets received", numRxDataPkt);
        recordScalar("Number of DATA packets received in GTS", numRxGTSPkt);
        recordScalar("Number of ACK packets received", numRxAckPkt);
        recordScalar("Number of COMMAND packets dropped due to concurrent processing", numRxCmdPktDropped);
        recordScalar("Number of DATA packets dropped due to concurrent processing", numRxDataPktDropped);
        recordScalar("Number of packets filtered in MAC", numRxPktFiltered);
        recordScalar("Number of collisions", numCollisions);
        recordScalar("Number of bit errors", numBitErrors);
    }
}
